...
namespace Repository.Repository
{
    public class MedicineRepository : IMedicineRepository
    {
        ...
        {
            return await context.Medicines.Include(k=>k.MedicineType).Include(w=>w.Frequency).AsNoTracking().SingleOrDefaultAsync(s => s.Id == id);
        }
        public async Task<IEnumerable<Medicine>> Get()
        {
            return await context.Medicines.Include(k => k.MedicineType).Include(w => w.Frequency).ToListAsync();
        }
